<template>
    <section class="article-comments">
        <div class="article__comment" v-for="comment in oneArticle.comment"
             :key="comment.id"
        >
            <a class="comment__delete" @click="deleteComment(comment.id)">
                <i class="fa-solid fa-trash-can article-card__icon"></i>
            </a>
            <a class="comment__edit"
               @click="[getOneComment([oneArticle.id, comment.id]),
               $router.push(`/article/${oneArticle.id}/comment/${comment.id}/edit`)]">
